...
<?= $this->section('title') ?>
<title>Detail Area Aset - SISKA14</title>
<?= $this->endSection() ?>
...
<section class="section">
    ...
    <div class="section-body">
        <div class="card author-box card-primary">
            ...
            <div class="card-body">
                <!-- TESSS -->
                <h3 id="title">Areal Aset : <?= $aset->nama_aset; ?></h3>
                <div class="row">
                    <div class="col-md-7">
                        <!-- Info -->
                        <table class="table table-bordered table-striped" style="width:100%">
                            <tbody>
                                <tr>
                                    ...
                                    <td style="width:70%"><?= $aset->nama_aset; ?></td>
                                </tr>
                                <tr>
                                    <td>Nama Pemilik</td>
                                ...
                                <tr>
                                    <td>Lokasi</td>
                                    <td>
                                        <?= $aset->nama_kelurahan ?>, <?= $aset->nama_kecamatan ?>, <?= $aset->nama_kabupaten ?>, <?= $aset->nama_provinsi ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Tanggal Terbit Sertifikat</td>
                                    <td><?= $aset->tgl_terbit ?></td>
                                </tr>
                                <tr>
                                    <td>Tanggal Berakhir Sertifikat</td>
                                    <td><?= $aset->tgl_expire ?></td>
                                </tr>
                                <tr>
                                    <td>Masa Berlaku Sertifikat</td>
                                    <td>
                                        <?php
                                        $lama_simpan_awal = $aset->tgl_terbit;
                                        $lama_simpan_akhir = $aset->tgl_expire;
                                        if ($lama_simpan_awal === '0000-00-00' || $lama_simpan_akhir === '0000-00-00' || empty($lama_simpan_awal) || empty($lama_simpan_akhir)) {
                                            ...
                                            if ($diff->y != 0) {
                                                echo "Masa berlaku : <b><i>" .  $diff->y . " tahun " . $diff->m . " bulan " . $diff->d . " hari</b></i>";
                                            } elseif ($diff->y == 0 && $diff->m != 0) {
                                                echo "Masa berlaku : <b><i>" . $diff->m . " bulan " . $diff->d . " hari</b></i>";
                                            } elseif ($diff->y == 0 && $diff->m == 0 && $diff->d != 0) {
                                                echo "Masa berlaku : <b><i>" . $diff->d . " hari</b></i>";
                                            } else {
                                                echo "<b><i>Masa berlaku : <b><i syle:'color:red;'>none</b></i>";
                                            }
                                        }
                                        ?>
                                    </td>
                                ...
                                <tr>
                                    <td>Luas aset (Ha)</td>
                                    <td>
                                        <?php echo number_format($aset->luas, 2, ",", ".") ?> Ha
                                    </td>
                                </tr>
                                <tr>
                                    <td>Keterangan</td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="gallery mt-2">
                                            <?php
                                            $gambar = explode(',', $aset->gambar);
                                            $i = 0;
                                            ?>
                                            <?php foreach ($gambar as $key2) { ?>
                                                <div class="gallery-item" data-image="<?= base_url() ?>/upload/aset/<?= $aset->id_aset ?>/file_gambar/<?= $gambar[$i] ?>" data-title="<?= $gambar[$i] ?>" href="<?= base_url() ?>/upload/aset/<?= $aset->id_aset ?>/file_gambar/<?= $gambar[$i] ?>" title="<?= $gambar[$i] ?>" style="background-image: url(&quot;<?= base_url() ?>/upload/aset/<?= $aset->id_aset ?>/file_gambar/<?= $gambar[$i] ?>" title="<?= $gambar[$i] ?>&quot;);"></div>
                                            <?php $i = $i + 1;
                                            } ?>
...
<script>
    ...
    <?php
    $file_string = $aset->file;
    $file = explode(',', $file_string);
    $i = 0;
    ?>
    <?php foreach ($file as $key2) { ?>
        $.getJSON('<?= base_url() ?>/upload/aset/<?= $aset->id_aset ?>/file_geo/<?= $file[$i] ?>', function(data) {
            aset<?= $i ?> = L.geoJSON(data, {
            ...
            }).bindPopup("<?= $aset->nama_aset ?>").addTo(aset);
        });
    <?php $i = $i + 1;
    } ?>
    ...
    const map = L.map('map', {
        center: [<?= $aset->koordinat ?>],
        zoom: 8,
        layers: [googleStreets, aset],
